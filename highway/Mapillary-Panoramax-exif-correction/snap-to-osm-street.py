# ============================================================
# Snap GPS photos to a named OSM street (indoor mapping helper)
# ============================================================
# Prérequis : pip install piexif pillow shapely requests tqdm numpy
# ============================================================
...
target_street_name  = "Rue de Louvain - Leuvenseweg"
...
def overpass_query(lat, lon, street_name):
    query = f"""
    [out:json];
    way["highway"]["name"="{street_name}"](around:100,{lat},{lon});
    out geom;
    """
    ...
    return {"elements": []}  # fallback vide, on skip la photo
...
for photo, path, coord in tqdm(zip(photos, paths, coords), total=len(photos)):
    ...
    lat, lon = coord
    data     = overpass_query(lat, lon, target_street_name)

    if not data.get("elements"):
        print(f"❌ Rue non trouvée pour {photo} ({lat:.6f}, {lon:.6f}) — photo ignorée.")
        continue

    new_lat, new_lon = snap_to_way(lat, lon, data)
    out_path         = os.path.join(output_folder, photo)

    try:
        update_exif_no_recompress(path, out_path, new_lat, new_lon)
        print(f"✅ {photo} → ({new_lat:.6f}, {new_lon:.6f})")
    except Exception as e:
